...
const ProfileSlider = () => {
    ...
    const getTokens = async () => {
        try {
            ...
            const _data = await axios.get("https://tgbot.bonomemebroker.com/api/maga").then(res => res.data).catch(() => [])

            setTradingTokens(_data)
            setLoading(false)
        } catch (error) {
    ...
    return (
        <section className="relative p-5 pl-[294px] slider  pr-[70px]  sm:mt-[135px] ">
                              ...
                              statistics rounded-[25px]  relative  flex-row items-center justify-between px-[24px] `} key={index}>
                                            ...
                                            <div className="flex flex-col ">
                                                ...
                                                <span className="text-[#E2EBFB] font-[400] text-[30px] leading-[30px] tracking-tight mb-[13px] font-american-x">
                                                    $
                                                    <CountUp start={0.0} end={parseFloat(item.worth.replaceAll(/,/g, "").replaceAll(/M/g, ""))} decimals={2} duration={2} />
                                                    M
                                                </span>
                                                {
                                                    item.percenntage < 0 ? <span className="text-[#E23F49] text-[14px] leading-[20px] font-[400] inline-flex items-center">
                                                        ...
                                                        <p>{item.percenntage.toFixed(2)}%</p>
            ...
            <img
                className='absolute right-[15%] hidden md:block  object-cover -top-[500px]'
                src="/assets/images/slider/bg-slider.png"
                alt="" />

            <div className='inline-flex text-[26px]  z-40 cursor-pointer gap-x-[50%] sm:gap-x-[52px] absolute w-[20%] sm:w-auto left-[40%] sm:left-auto -bottom-[40px] sm:bottom-auto sm:-top-[50px] sm:right-[440px]'>
